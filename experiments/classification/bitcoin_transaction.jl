...
#-------------------------
va_lp    = Vector(undef, length(ηs));
va_lgc   = Vector(undef, length(ηs));
va_lgcrp = Vector(undef, length(ηs));
va_sgcrp = Vector(undef, length(ηs));
va_gcnrp = Vector(undef, length(ηs));
#-------------------------
...
va_lp[i]    = run_dataset(G, feats, labels, dtr, dva, feature_smoothing=false, predictor="mean", residual_propagation=true,  η=η,             n_step=n_step, return_predictions=true);
va_lgc[i]   = run_dataset(G, feats, labels, dtr, dva, feature_smoothing=true,  predictor="mlp",  residual_propagation=false, η=η, σ=identity, n_step=n_step, return_predictions=true);
va_lgcrp[i] = run_dataset(G, feats, labels, dtr, dva, feature_smoothing=true,  predictor="mlp",  residual_propagation=true,  η=η, σ=identity, n_step=n_step, return_predictions=true);
va_sgcrp[i] = run_dataset(G, feats, labels, dtr, dva, feature_smoothing=false, predictor="gcn",  residual_propagation=true,  η=η, σ=identity, n_step=n_step, return_predictions=true);
va_gcnrp[i] = run_dataset(G, feats, labels, dtr, dva, feature_smoothing=false, predictor="gcn",  residual_propagation=true,  η=η, σ=relu,     n_step=n_step, return_predictions=true);
end
#-------------------------
...
te_lp    = run_dataset(G, feats, labels, dtr, dte, feature_smoothing=false, predictor="mean", residual_propagation=true,  η=ηs[argmax((yy -> optimal_F1(yy)[1]).(va_lp))],                   n_step=n_step, return_trace=true, return_predictions=true);
te_lgc   = run_dataset(G, feats, labels, dtr, dte, feature_smoothing=true,  predictor="mlp",  residual_propagation=false, η=ηs[argmax((yy -> optimal_F1(yy)[1]).(va_lgc))],   σ=identity, n_step=n_step, return_trace=true, return_predictions=true);
te_lgcrp = run_dataset(G, feats, labels, dtr, dte, feature_smoothing=true,  predictor="mlp",  residual_propagation=true,  η=ηs[argmax((yy -> optimal_F1(yy)[1]).(va_lgcrp))], σ=identity, n_step=n_step, return_trace=true, return_predictions=true);
te_sgcrp = run_dataset(G, feats, labels, dtr, dte, feature_smoothing=false, predictor="gcn",  residual_propagation=true,  η=ηs[argmax((yy -> optimal_F1(yy)[1]).(va_sgcrp))],   σ=identity, n_step=n_step, return_trace=true, return_predictions=true);
te_gcnrp = run_dataset(G, feats, labels, dtr, dte, feature_smoothing=false, predictor="gcn",  residual_propagation=true,  η=ηs[argmax((yy -> optimal_F1(yy)[1]).(va_gcnrp))],    σ=relu,     n_step=n_step, return_trace=true, return_predictions=true);
