    ...
    #-----------------------------------------------------------------------------------
        ...
        #-------------------------------------------------------------------------------
            for seed_val in 1:ntrials
                ...
                ac[i,j,seed_val] = run_dataset(G, feats, labels, dtr, dte, feature_smoothing=true, predictor="mlp", residual_propagation=false, σ=identity, η=ω/(1.0+ω), n_step=300, seed_val=seed_val)[end];
            end
        #-------------------------------------------------------------------------------
        end
        #-------------------------------------------------------------------------------
...
ac_lgc = demo_synthetic("large");
writedlm("results/lgc_accuracy.csv", ac_lgc);
# ac_lgc = readdlm("results/lgc_accuracy.csv");

using Plots; pyplot();
using LaTeXStrings;

cmap = cgrad(:balance);
min_value = minimum(ac_lgc);
max_value = maximum(ac_lgc);
...
h = Plots.plot(size=(320,270), title="LGC accuracy (" * L"R^{2}" * ")", xlabel="Gaussian MRF "*L"\omega", ylabel="LGC "*L"\omega", framestyle=:box);
xx = collect(-1.00:0.25:2.00);
yy = collect(-1.00:0.25:2.00);
Plots.contour!(h, xx, yy, ac_lgc', fill=true, seriescolor=cmap, levels=20,
...
Plots.scatter!(h, xx, yy[[idx[2] for idx in argmax(ac_lgc, dims=2)]], markersize=6.0, markercolor=:yellow, label=nothing);
Plots.savefig(h, "figs/lgc_accuracy.pdf")
Plots.savefig(h, "figs/lgc_accuracy.svg")
